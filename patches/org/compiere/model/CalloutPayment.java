...
public class CalloutPayment extends CalloutEngine
{
		...
		GridField mField, Object value)
	{
		...
		if (Env.getContextAsInt (ctx, WindowNo, Env.TAB_INFO, "C_Order_ID") == C_Order_ID.intValue ()
			&& Env.getContextAsInt (ctx, WindowNo, Env.TAB_INFO, "CUST_OrderPrePaySchedule_ID") != 0)
		{
			CUST_OrderPrePaySchedule_ID = Env.getContextAsInt (ctx, WindowNo, Env.TAB_INFO, "CUST_OrderPrePaySchedule_ID");
		}
		// Payment Date
		Timestamp ts = (Timestamp)mTab.getValue ("DateTrx");
		if (ts == null)
		...
		try
		{
			...
			pstmt.setTimestamp (2, ts);
			pstmt.setInt (3, CUST_OrderPrePaySchedule_ID);
